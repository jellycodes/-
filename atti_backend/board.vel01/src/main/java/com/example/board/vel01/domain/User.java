...
public class User {
	...
	public static class Request {
		...
		public static User toEntity(final Request request) {
			return User.builder().id(request.getId()).nickName(request.getNickName()).pwd(request.getPwd())
					.posts(new ArrayList<>()).comments(new ArrayList<>()).build();
		}
	}
	...
	public static class Response {
		...
		public static User.Response toResponse(final User user) {
			return User.Response.builder().id(user.getId()).nickName(user.getNickName()).joinDate(user.getJoinDate())
					.posts(Post.Response.toResponseList(user.getPosts()))
					.comments(Comment.Response.toResponseList(user.getComments())).token(user.getToken())
